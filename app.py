...
def generate_parent_message(student_name, score, is_absent):
    if is_absent:
        return f"المكرم ولي أمر الطالب/ {student_name}، نود التنبيه على غياب الطالب هذا الأسبوع، ونحثكم على متابعة الأنتظام وحضوره الاختبارات لتجنب حسم الدرجات والتأثير على مستواه الدراسي ."
...
، نرجو منكم تكثيف المتابعة المنزلية والتواصل معنا للوقوف على أسباب هذا التراجع ووضع خطة لتحسين أدائه.مع تحياتنا
 ({sc_str}) وهي أقل من 50%. نأمل منكم المتابعة والعمل على رفع مستواه الدراسي."
    elif score <= 75:
        return f"المكرم ولي أمر الطالب/ {student_name}، ،نود إحاطتكم علماً بأن المستوى التحصيلي لابنكم جيد ومستقر بشكل عام، ولكنه يمتلك قدرات أعلى تؤهله لتحقيق درجات أفضل.نأمل منكم التركيز معه في الفترة القادمة  لرفع كفاءته الدراسية.شاكرين لكم تعاونكم الدائم.مع تحياتنا ({score}%)، ونحثه على بذل المزيد من الجهد للوصول إلى مستوى أفضل والتميز."
    else:
        return f"المكرم ولي أمر الطالب/ {student_name}، يسعدنا إبلاغكم بأن ابنكم قدم أداءً تحصيلياً متميزاً وسلوكاً رائعاً داخل الفصل، وحصل على درجات ممتازة في التقييمات الأخيرة.نشكر لكم حسن المتابعة والاهتمام، ونرجو الاستمرار في هذا الدعم المتبادل للحفاظ على هذا المستوى المتفوق.مع تحياتنا ({score}%)، مع أطيب دعواتنا له بالتوفيق والاستمرارية."
